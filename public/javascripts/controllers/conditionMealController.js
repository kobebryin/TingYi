angular.module('TinYi').controller('conditionMealController', function ($rootScope, $scope, $timeout, MemberService, monthMealService) {
        ...
        MemberService.getOneMEMBER(id, function (data) {
            ...
            if ($scope.mealForMember.meal2ac != null) {
                if ($scope.mealForMember.meal2ac.length > 9) {
                    ...
                    $scope.MealXARemain = - meal2ac_initial_array.length; //剩餘參數塞值
                    if ($scope.mealForMember.meal2a !== null) $scope.MealXARemain = - meal2ac_initial_array.length + $scope.mealForMember.meal2a;
                } else {
                    $scope.MealXARemain = 0;
                    if ($scope.mealForMember.meal2a !== null) $scope.MealXARemain = 0 + $scope.mealForMember.meal2a;
                }
            } else {
                $scope.MealXARemain = 0;
                if ($scope.mealForMember.meal2a !== null) $scope.MealXARemain = 0 + $scope.mealForMember.meal2a;
            ...
            if ($scope.mealForMember.meal2bc != null) {
                if ($scope.mealForMember.meal2bc.length > 9) {
                    ...
                    $scope.MealXBRemain = - meal2bc_initial_array.length; //剩餘參數塞值
                    if ($scope.mealForMember.meal2b !== null) $scope.MealXBRemain = - meal2bc_initial_array.length + $scope.mealForMember.meal2b;
                } else {
                    $scope.MealXBRemain = 0;
                    if ($scope.mealForMember.meal2b !== null) $scope.MealXBRemain = 0 + $scope.mealForMember.meal2b;
                }
            } else {
                $scope.MealXBRemain = 0;
                if ($scope.mealForMember.meal2b !== null) $scope.MealXBRemain = 0 + $scope.mealForMember.meal2b;
            ...
            if ($scope.mealForMember.meal2cc != null) {
                if ($scope.mealForMember.meal2cc.length > 9) {
                    ...
                    $scope.MealXCRemain = - meal2cc_initial_array.length; //剩餘參數塞值
                    if ($scope.mealForMember.meal2c !== null) $scope.MealXCRemain = - meal2cc_initial_array.length + $scope.mealForMember.meal2c;
                } else {
                    $scope.MealXCRemain = 0;
                    if ($scope.mealForMember.meal2c !== null) $scope.MealXCRemain = 0 + $scope.mealForMember.meal2c;
                }
            } else {
                $scope.MealXCRemain = 0;
                if ($scope.mealForMember.meal2c !== null) $scope.MealXCRemain = 0 + $scope.mealForMember.meal2c;
    ...
    $scope.saveMeal = function() {
        ...
        MemberService.putMEMBERMEALforConditionMeal(mealForMember, function (data) {
            initial();
            $.LoadingOverlay("hide");
        });
    };
        ...
        if (month_calendar.multiDatesPicker('getDates').length === 0) {
            ...
            monthMealService.show_data(inputObj, function (data) {
                if (data.length > 0) {
                    ...
                    var attrib05Array = [];
                    if ($scope.meallistA.meal06 !== null) attrib05Array = $scope.meallistA.meal06.split(",");
            ...
            monthMealService.show_data(inputObj, function (data) {
                if (data.length > 0) {
                    ...
                    var attrib05setArray = [];
                    var attrib05Array = [];
                    if ($scope.meallistB.meal06 !== null) attrib05Array = $scope.meallistB.meal06.split(",");
                    for (key in attrib05Array) {
                        attrib05setArray.push({ id: attrib05Array[key], text: attrib05Array[key] });
                    }
            ...
            monthMealService.show_data(inputObj, function (data) {
                if (data.length > 0) {
                    ...
                    var attrib05setArray = [];
                    var attrib05Array = [];
                    if ($scope.meallistC.meal06 !== null) attrib05Array = $scope.meallistC.meal06.split(",");
                    for (key in attrib05Array) {
                        attrib05setArray.push({ id: attrib05Array[key], text: attrib05Array[key] });
                    }
        ...
        if (month_calendar_morning.multiDatesPicker('getDates').length === 0 && month_calendar_noon.multiDatesPicker('getDates').length === 0 && month_calendar_night.multiDatesPicker('getDates').length === 0 && month_calendar.multiDatesPicker('getDates').length === 0) {
            ...
            if (month_calendar_morning.multiDatesPicker('getDates').length === 0) {
                ...
                monthMealService.show_data(inputObj, function (data) {
                    ...
                    // ContentStr += "調理內容 : " + data[0].Meal05 + "\n";
                    // ContentStr += "禁忌 : " + data[0].Meal06 + "\n";
                    // ContentStr += "加減 : " + data[0].Meal07 + "\n";
                    // ContentStr += "換飲 : " + data[0].Meal08 + "\n";
                    // ContentStr += "收款 : " + data[0].Meal09 + ", " + data[0].Meal10 + "元\n";
                    // ContentStr += "提前到 : " + data[0].Meal11 + ", " + data[0].Meal12 + " : " + data[0].Meal13 + "\n";
                    // ContentStr += "放警衛室 : " + data[0].Meal14 + "\n";
                    // ContentStr += "備註 : " + data[0].Meal15 + "\n";
        ...
        if (month_calendar_morning.multiDatesPicker('getDates').length === 0 && month_calendar_noon.multiDatesPicker('getDates').length === 0 && month_calendar_night.multiDatesPicker('getDates').length === 0) {
            ...
            if (month_calendar_morning.multiDatesPicker('getDates').length === 0) {
                ...
                for (let key in dates_A) {
                    ...
                    monthMealService.edit_overwrite_Meal2(edit_join_data, function (data) {
                        initial();
                        if (key == dates_A.length - 1) $.LoadingOverlay("hide");
                    });
                }
    ...
    $scope.edit_join = function () {
        if (month_calendar_morning.multiDatesPicker('getDates').length === 0 && month_calendar_noon.multiDatesPicker('getDates').length === 0 && month_calendar_night.multiDatesPicker('getDates').length === 0) {
            ...
            if (month_calendar_morning.multiDatesPicker('getDates').length === 0) {
                ...
                monthMealService.show_data(inputObj, function (data) {
                    var editJoin_meal06 = data[0].Meal06.split(',');
                    var editJoin_meal07 = data[0].Meal07.split(',');

                    if ($('#Meal_A_Meal06').val() != '') {
                        var deleteArray = $('#Meal_A_Meal06').val().split(',');
                        for (let key in deleteArray) {
                            if (!editJoin_check(editJoin_meal06, deleteArray[key])) {
                                editJoin_meal06.push(deleteArray[key]);
                            } else {
                                //原本陣列內有相同的就不做加入
                            }
                        }
                        $scope.meallistA.meal06 = editJoin_meal06.join(',');
                    }
                    if ($('#Meal_A_Meal07').val() != '') {
                        var deleteArray = $('#Meal_A_Meal07').val().split(',');
                        for (let key in deleteArray) {
                            if (!editJoin_check(editJoin_meal07, deleteArray[key])) {
                                editJoin_meal07.push(deleteArray[key]);
                            } else {
                                //原本陣列內有相同的就不做加入
                            }
                        }
                        $scope.meallistA.meal07 = editJoin_meal07.join(',');
                    }
                    ...
                    for (let key in dates_A) {
                        ...
                        monthMealService.edit_join_Meal2(edit_join_data, function (data) {
                            if (data === '未輸入任何修改資料!') {
                                alert(data);
                                $.LoadingOverlay("hide");
                            } else {
        ...
        if (month_calendar_morning.multiDatesPicker('getDates').length === 0 && month_calendar_noon.multiDatesPicker('getDates').length === 0 && month_calendar_night.multiDatesPicker('getDates').length === 0) {
            ...
            if (month_calendar_morning.multiDatesPicker('getDates').length === 0) {
                ...
                for (let key in dates_A) {
                    ...
                    monthMealService.S2_deleteMeal_A(delete_data, function (data) {
                        initial();
                        if (key == dates_A.length - 1) $.LoadingOverlay("hide");
                    });
                }
                ...
                // console.log(dates);
                var set1 = new Set(meal2ac_initial_array);
                ...
                if (subset.length === 0) {
                    ...
                    monthMealService.S2_putMeal_A(inputObj, function (data) {
                        initial();
                    });
                } else {
                    for (key in subset) {
                        meal2ac_string += subset[key] + ";";
                    }

                    let inputObj = {
                        id: sessionStorage.memberid,
                        mea2ac: meal2ac_string
                    }
                    monthMealService.S2_putMeal_A(inputObj, function (data) {
                        initial();
                    });
            ...
            if (month_calendar_noon.multiDatesPicker('getDates').length === 0) {
                ...
                for (let key in dates_B) {
                    ...
                    monthMealService.S2_deleteMeal_B(delete_data, function (data) {
                        initial();
                        if (key == dates_B.length - 1) $.LoadingOverlay("hide");
                    });
                }
                ...
                // console.log(dates);
                var set1 = new Set(meal2bc_initial_array);
                var set2 = new Set(dates);
                ...
                if (subset.length === 0) {
                    ...
                    monthMealService.S2_putMeal_B(inputObj, function (data) {
                        initial();
                    });
                } else {
                    for (key in subset) {
                        meal2bc_string += subset[key] + ";";
                    }

                    let inputObj = {
                        id: sessionStorage.memberid,
                        mea2bc: meal2bc_string
                    }
                    monthMealService.S2_putMeal_B(inputObj, function (data) {
                        initial();
                    });
            ...
            if (month_calendar_night.multiDatesPicker('getDates').length === 0) {
                ...
                for (let key in dates_C) {
                    ...
                    monthMealService.S2_deleteMeal_C(delete_data, function (data) {
                        initial();
                        if (key == dates_C.length - 1) $.LoadingOverlay("hide");
                    });
                }
                ...
                // console.log(dates);
                var set1 = new Set(meal2cc_initial_array);
                var set2 = new Set(dates);
                ...
                if (subset.length === 0) {
                    ...
                    monthMealService.S2_putMeal_C(inputObj, function (data) {
                        initial();
                    });
                } else {
                    for (key in subset) {
                        meal2cc_string += subset[key] + ";";
                    }

                    let inputObj = {
                        id: sessionStorage.memberid,
                        mea2cc: meal2cc_string
                    }
                    monthMealService.S2_putMeal_C(inputObj, function (data) {
                        initial();
                    });
    ...
    $scope.onChange = function () {
        ...
        Edit_Type = $('[name="Edit_Type_1"]:checked').val();
        if (Edit_Type === '1') {
